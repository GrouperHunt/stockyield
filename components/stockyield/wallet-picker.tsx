...
export function WalletPicker({ open, onOpenChange, wallets, onPick }: { open: boolean; onOpenChange: (o: boolean) => void; wallets: EIP6963ProviderDetail[]; onPick: (p: EIP1193Provider) => void }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-md bg-surface sm:max-w-sm">
        ...
        <div className="space-y-2">
          {wallets.map((w) => (
            <button key={w.info.uuid} onClick={() => { onOpenChange(false); onPick(w.provider); }} className="flex w-full items-center gap-3 border bg-surface p-3 text-left hover:bg-surface-2">
